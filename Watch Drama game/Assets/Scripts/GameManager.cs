...
public class GameManager : MonoBehaviour
{
    ...
    [Title("Show Ending Panel")]
    [Button("🎬 Show Ending Panel (All Maps Complete)", ButtonSizes.Large)]
    [GUIColor(1f, 0.5f, 0f)]
    public void ShowEndingPanelDebug() => ShowEndingPanelDirectly(EndingScenario.AllMapsCompleted);
    
    [Button("🎬 Show Trust Victory Ending", ButtonSizes.Large)]
    [GUIColor(0.2f, 0.8f, 0.2f)]
    public void ShowTrustEndingDebug() => ShowEndingPanelDirectly(EndingScenario.TrustVictory);
    
    [Button("🎬 Show Faith Victory Ending", ButtonSizes.Large)]
    [GUIColor(0.2f, 0.8f, 1f)]
    public void ShowFaithEndingDebug() => ShowEndingPanelDirectly(EndingScenario.FaithVictory);
    
    [Button("🎬 Show Hostility Victory Ending", ButtonSizes.Large)]
    [GUIColor(1f, 0.2f, 0.2f)]
    public void ShowHostilityEndingDebug() => ShowEndingPanelDirectly(EndingScenario.HostilityVictory);
    
    [Button("🎬 Show Balanced Victory Ending", ButtonSizes.Large)]
    [GUIColor(1f, 1f, 0.2f)]
    public void ShowBalancedEndingDebug() => ShowEndingPanelDirectly(EndingScenario.BalancedVictory);
    ...
    /// <summary>
    /// Directly show the ending panel with a specific scenario (Debug)
    /// </summary>
    private void ShowEndingPanelDirectly(EndingScenario scenario)
    {
        // First, complete all maps
        CompleteAllMaps();
        
        // Find the EndingPanelUI and show the ending
        var endingPanelUI = UnityEngine.Object.FindFirstObjectByType<EndingPanelUI>(FindObjectsInactive.Include);
        if (endingPanelUI != null)
        {
            Debug.Log($"🎬 DEBUG: Showing ending panel with scenario: {scenario}");
            endingPanelUI.ShowEnding(scenario);
            
            // Also trigger the game finished event
            OnGameFinished?.Invoke(scenario);
        }
        else
        {
            Debug.LogError("🎬 DEBUG: EndingPanelUI not found in scene! Make sure it exists.");
        }
    }
