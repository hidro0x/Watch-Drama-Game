...
public class ChoiceSelectionUI : MonoBehaviour
{
                ...
                characterImage.rectTransform.DOAnchorPos(new Vector2(screenWidth, 0), ANIMATION_DURATION).SetEase(Ease.InQuad)
                    .OnComplete(() => {
                        ...
                        if (dialogueNode is DialogueNode dn2 && dn2.isGlobalDialogue)
                        {
                            ...
                            AnimateTextReveal();
                            choicesPanel.DOAnchorPos(new Vector2(0, 0), ANIMATION_DURATION).SetEase(Ease.OutQuad);
                            // bottomPanel stays down during dialogue transitions
    ...
    public void OnPanelClosed()
    {
        ...
        // Note: bottomPanel will be moved back up when returning to main screen after country completion
    }
    
    /// <summary>
    /// Move the bottomPanel back up instantly (called when returning to main screen after country completion)
    /// </summary>
    public void MoveBottomPanelUp()
    {
        if (bottomPanel != null)
        {
            // Ensure the bottomPanel's GameObject is active
            if (!bottomPanel.gameObject.activeInHierarchy)
            {
