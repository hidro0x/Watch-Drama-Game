...
public class DialogueManager : MonoBehaviour
{
    ...
    public void UpdateTurnText()
    {
        if (turnText != null)
        {
            turnText.text = $"{SimpleRuntimeLocalizer.Instance.GetText("ui_turn")}: {currentTurn}";
        }
    }
    ...
    void ShowDialogue(DialogueNode node)
    {
        ...
        DialogueNode localizedNode = node;
        if (DialogueLocalizationManager.Instance != null)
        {
            localizedNode = DialogueLocalizationManager.Instance.GetLocalizedDialogue(node);
        }
        
        // ChoiceSelectionUI ile entegre gösterim
        if (choiceSelectionUI != null)
        {
            choiceSelectionUI.ShowUI(localizedNode);
        }
        else
        {
